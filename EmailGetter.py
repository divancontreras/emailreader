...
while True:
	...
	for message in messages:
		...
		if subjectGet in message.Subject and emailGet in msgSender and (subject2Get in message.Subject):
			for x in range(0,5):
				if message.Subject.split()[x] != '***' and message.Subject.split()[x] != '//' and message.Subject.split()[x] != 'SCHNEIDER' and message.Subject.split()[x] != 'ELECTRIC':
					factura = message.Subject.split()[x]
					break
			for x in range(1,row):
				if ws.cell(row=x,column=6).value == factura:
					fecha = message.SentOn.split()[0]
					hora = message.SentOn.split()[1]
					ws.cell(row=row,column=18).value = fecha
					ws.cell(row=row,column=19).value = hora
		...
		if subject in message.Subject and email in message.SenderEmailAddress and not(subjectAd in message.Subject):
			...
			ws.cell(row=row, column=15).value = message.SentOn
	...
	Input = input("Enter para refrescar / S para Salir: ")
	if(Input.lower() == 's'):
		exit()